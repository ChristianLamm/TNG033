...
double calculate(double k) {
    ...
    std::transform(other_factors.begin(), other_factors.end(),
        other_factors.begin(), [&factor](double d) { return -1.0 / (d + factor); }); //ersätter forloop
...
int main() {
    ...
    std::iota(terms.begin(), terms.end(), 0); //push_back funktion för stl


    std::vector<double> power_terms{ terms };

    std::transform(terms.begin(), terms.end(), terms.begin(), calculate); //går igenom varje element och applicerar calc

    std::transform(power_terms.begin(), power_terms.end(), //sammma sak fast med andra parentesen
        power_terms.begin(), [](int i) { return std::pow(16.0, -i); }); //nu har vi två vektorer att beräkna

    double result{ 0.0 };
    result = std::inner_product(terms.begin(), terms.end(), power_terms.begin(), 0.0); //beräknar skalärprodukten mellan
